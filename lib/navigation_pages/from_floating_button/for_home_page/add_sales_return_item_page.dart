...
import 'package:offline_billing/database/sales_return_item_db.dart';
import 'package:offline_billing/database/unit_db.dart';

class AddSalesReturnItemPage extends StatefulWidget {
  const AddSalesReturnItemPage({super.key});

  @override
  State<AddSalesReturnItemPage> createState() => _AddSalesReturnItemPageState();
}

class _AddSalesReturnItemPageState extends State<AddSalesReturnItemPage> {
  ...
  void didChangeDependencies() {
    ...
    if (args != null) {
      originalSalesId = args['original_sales_id'];
      final isEditing = args['isEditing'] ?? false;

      if (isEditing) {
        // Editing existing item - load all data from args
        _prefillData(args);
      } else if (originalSalesId != null) {
        // New item with original sales ID - load original items
        _loadOriginalItems(originalSalesId!);
      }
    }
  }
  ...
  Future<void> _loadItemData(int itemId) async {
    ...
    try {
      // First try to load from SalesReturnItemDB
      final returnItem = await SalesReturnItemDB.getCompleteReturnItemById(
        itemId,
      );
      _prefillData(returnItem);
      originalItemId = itemId;

      // Then load the original sales item to get max returnable quantity
      if (returnItem['original_item_id'] != null) {
        final originalItem = await SalesItemDB.getCompleteSalesItemById(
  ...
  Future<void> _loadOriginalItems(int salesId) async {
    setState(() => isLoading = true);
    try {
      final items = await SalesItemDB.getSalesItemsBySalesId(salesId);
      ...
      if (items.isNotEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No items found in the original sale')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(
  ...
  Future<void> _selectOriginalItemDialog() async {
    if (originalItems.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No items available in the original sale'),
        ),
      );
      return;
    }
