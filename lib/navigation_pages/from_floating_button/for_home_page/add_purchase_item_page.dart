...
class AddPurchaseItemPage extends StatefulWidget {
  const AddPurchaseItemPage({super.key});
...
class _AddPurchaseItemPageState extends State<AddPurchaseItemPage>
  ...
  void _prefillData(Map<String, dynamic> itemData) {
    setState(() {
      // Preserve all existing data including IDs
      ...
      // Tax percentage handling with proper fallbacks
      final taxPercent = itemData['tax_percent'] ?? 'GST@ 0%';
      if (taxPercent is String) {
        _taxPercentController.text =
            taxPercent.contains('%') ? taxPercent : 'GST@ ${taxPercent}%';
      } else {
        _taxPercentController.text = 'GST@ ${taxPercent.toStringAsFixed(0)}%';
      }

      // Always initialize all calculation fields with proper fallbacks
      _subtotalController.text = (itemData['subtotal'] ?? 0).toString();
      _discountPercentController.text =
          (itemData['discount_percent'] ?? 0).toString();
      _discountValueController.text =
          (itemData['discount_value'] ?? 0).toString();
      _taxValueController.text = (itemData['tax_value'] ?? 0).toString();
      _totalAmountController.text = (itemData['total_amount'] ?? 0).toString();

      // Force calculations to ensure consistency
      _calculateSubtotal();
    ...
    await showModalBottomSheet(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    "Cost Price : ${product['costPrice']?.toStringAsFixed(2) ?? '0.00'}",
                                  ),
                                  Text(
      ...
      if (value != null && value is Map<String, dynamic>) {
        ...
        setState(() {
          ...
          _rateController.text = product['costPrice']?.toString() ?? '0';
          _calculateSubtotal();
        });
      }
    });
  ...
  Widget _buildSaveButton() {
    return ElevatedButton(
      onPressed: () async {
        ...
        // Prepare complete data for UI return with all fields
        final itemDataForUI = <String, dynamic>{
          'id':
              _tempTag != null
                  ? _tempTag!.replaceAll('temp_', '')
                  : null, // Preserve ID if exists
          ...
          'temp_tag': _tempTag ?? _generateTempTag(),
        };
        ...
        final itemDataForDB = <String, dynamic>{
          ...
          'tax_percent':
              _taxPercentController.text, // Store as string to preserve format
        ...
        try {
          if (_tempTag != null) {
            ...
            final existingItems = await PurchaseItemDB.getItemsByTempTag(
              _tempTag!,
            );

            if (existingItems.isNotEmpty) {
              // Update existing item with complete data
              await PurchaseItemDB.updateItem({
                ...itemDataForDB,
                'id': existingItems.first['id'],
                'temp_tag': _tempTag,
              });
              itemDataForUI['id'] =
                  existingItems.first['id']; // Ensure ID is passed back
            } else {
              // Insert as new item if not found
              final newTempTag = _generateTempTag();
              await PurchaseItemDB.insertItem({
                ...itemDataForDB,
                'temp_tag': newTempTag,
              });
          ...
          // Return complete item data including tax info
