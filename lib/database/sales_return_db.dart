...
class SalesReturnDB {
  static const String tableName = 'sales_return';

  // Create the sales_return table
  static Future<void> createTable(Database db) async {
    await db.execute('''
      CREATE TABLE $tableName (
        ...
        return_no TEXT,
        date TEXT,
        original_sales_id INTEGER,
        original_sales_no TEXT,
        customer_name TEXT, 
        total_amount REAL,
        payment_status TEXT,
        payment_method TEXT,
        remarks TEXT,
        FOREIGN KEY (original_sales_id) REFERENCES sales(id)
      )
    ''');
  }
  ...
  static Future<int> deleteReturn(int returnId) async {
    ...
    return await db.transaction((txn) async {
      ...
      if (returnDeleteResult == 1 && items.isNotEmpty) {
        // Adjust stock for each item (decrease since we're undoing a return)
  ...
  static Future<void> _adjustStockAfterDeletion(
    ...
    if (products.isNotEmpty) {
      ...
      final newStock =
          currentStock - quantity; // Decrease stock (opposite of return)
  ...
  // Get returns by original sales ID
  static Future<List<Map<String, dynamic>>> getReturnsByOriginalSalesId(
    int salesId,
  ) async {
    final db = await DatabaseHelper.instance.database;
    return await db.query(
      tableName,
      where: 'original_sales_id = ?',
      whereArgs: [salesId],
      orderBy: 'date DESC',
    );
  }
